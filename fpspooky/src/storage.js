...
function loadConfig() {
    ...
    let settings = {
        ...
        reviewText: config.get('FunPay', 'reviewText'),
        ...
        bot: {
            ...
            orderName: config.get('Bot', 'orderName'),
            chatActive: Number(config.get('Bot', 'chatActive')),
            anarchy: Number(config.get('Bot', 'anarchy')),
            notGiveTimeout: Number(config.get('Bot', 'notGiveTimeout'))
        }
    };
...
async function saveConfig(settings) {
    ...
    data = setValue(data, 'FunPay', 'reviewText', settings.reviewText);
    ...
    data = setValue(data, 'Bot', 'username', settings.bot.username);
    data = setValue(data, 'Bot', 'anarchy', settings.bot.anarchy);
    data = setValue(data, 'Bot', 'chatActive', settings.bot.chatActive);
    data = setValue(data, 'Bot', 'orderName', settings.bot.orderName);
    data = setValue(data, 'Bot', 'notGiveTimeout', settings.bot.notGiveTimeout);
...
function getConst(name) {
    switch (name) {
        ...
        case 'autoIssueFilePath':
            return `${dataPath}/configs/delivery.json`;
        case 'chatId':
